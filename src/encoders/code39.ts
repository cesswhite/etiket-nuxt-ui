...
export function encodeCode39(
  ...
  if (text.includes('*')) {
    throw new InvalidInputError(
      'Code 39 input must not contain the start/stop character (*)',
    )
  }
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (index === undefined) {
      throw new InvalidInputError(
        `Invalid Code 39 character: '${ch}' at position ${i}`,
      )
    }
    indices.push(index)
  }
 ...
 * @returns Array of bar widths (alternating bar/space)
 */
export function encodeCode39Extended(
  text: string,
  options?: { checkDigit?: boolean },
): number[] {
  if (text.length === 0) {
