...
export function encodeASCII(text: string): number[] {
  ...
  while (i < text.length) {
    ...
    if (charCode > 255) {
      throw new InvalidInputError(`Data Matrix ASCII mode does not support character: "${text[i]}" (U+${charCode.toString(16).padStart(4, '0')})`)
    }

    // Check for digit pair optimization
    if (
      charCode >= 48 && charCode <= 57 && // current char is '0'-'9'
      i + 1 < text.length &&
      text.charCodeAt(i + 1) >= 48 && text.charCodeAt(i + 1) <= 57 // next char is '0'-'9'
    ) {
