...
export function optimizeSegments(text: string, version: number): QRSegment[] {
  ...
  for (let i = 1; i <= text.length; i++) {
    ...
    const shouldSwitch = i === text.length || shouldSwitchMode(currentMode, charMode, text, i, version)
    ...
    if (shouldSwitch || i === text.length) {
      ...
      segments.push({
        ...
        charCount: currentMode === 'byte' ? new TextEncoder().encode(segText).length : segText.length,
      })
