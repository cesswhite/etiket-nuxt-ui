...
export function renderQRCodeSVG(
  ...
  if (corners) {
    for (const fp of finderPositions) {
      ...
      parts.push(`<path d="${getFinderOuterPath(x, y, moduleSize, outerShape)}" fill="${outerColor}" fill-rule="evenodd"/>`)
      ...
      parts.push(`<path d="${getFinderInnerPath(x, y, moduleSize, innerShape)}" fill="${innerColor}"/>`)
    }
  } else {
